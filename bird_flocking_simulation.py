...
#this function is a problem because it allows for repeat numbers. no two birds can occupy the same space. 
def create_birds():
    x_val_array = []
    y_val_array = []
    birds = []

    while len(x_val_array) < 101:
        i = random.randint(0,101)
        x_val_array.append(i)
    while len(y_val_array) < 101:
        i = random.randint(0,101)
